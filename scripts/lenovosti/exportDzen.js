function exportDzen() {
	...
	//change view
	changeView('exportDzen');

	//clear view
	$('#exportDzenFullSize').html('');
	...
	html += '<br><br><h3>' + dateObj.todayByWordRus + '</h3>';
	...
	if (todayRates.length > 0 && yesterdayRates.length > 0) {
		html += '<a href="https://quote.ru/ticker/72413">Доллар</a> ' + displayRates(yesterdayRates[0].usd, todayRates[0].usd) + ' ₽';
	...
	//HQ
	html += '<br><br><h3>Штаб докладывает</h3>';
	...
	if (dateObj.todayDayOfWeekWord == 'Friday') {
		html += '<br><br><h3>Следующий выпуск в понедельник</h3>';
	}

	//write HTML
	$('#exportDzenFullSize').append(html);
